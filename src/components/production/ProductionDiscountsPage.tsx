...
export function ProductionDiscountsPage() {
  ...
  const list = useListQueryState({ syncUrl: true, filterKeys: ['status', 'branchId', 'businessDate'] });
  const branchesQ = useBranches(token);
  const filters = useMemo<FilterConfig[]>(
    () => [
      {
        key: 'status', label: 'Status', type: 'select', placement: 'bar',
        options: (Object.keys(STATUS_LABELS) as BranchDiscountStatus[]).map((s) => ({ value: s, label: STATUS_LABELS[s] })),
      },
